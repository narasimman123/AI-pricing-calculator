...
import { MenuItem, FormControl, InputLabel, Select, Container, Box, Typography, AppBar, Toolbar, Button, Snackbar, Alert, TextField, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton } from '@mui/material';
...
const getAverageTokens = (model, topK) => {
  ...
  switch (model) {
    ...
    case 'Pricing not available': // GPT-4-Turbo-128K
      return (
    ...
    case 'NA': // GPT-3.5-Turbo-0125-16K
      return (
 ...
 const getAverageOutputTokens = (model, topK) => {
  ...
  const handleInputChange = (index, field, value) => {
    ...
    // Update Avg Tokens2 when Avg Words2 or Posts change
    if (field === 'avgWords2' || field === 'posts') {
      updatedSets[index].avgTokens2 = calculateOutputTokens(updatedSets[index].avgWords2, updatedSets[index].posts);
    }
      ...
      <AppBar position="static">
        <Toolbar>
          <Box display="flex" justifyContent="center" width="100%">
            <Typography variant="h6">Gen AI Pricing</Typography>
          </Box>
        </Toolbar>
      </AppBar>
